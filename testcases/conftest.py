...
role_data = get_data("role_data.yml")
employee_data = get_data("employee_data.yml")
...
def step_login(username,token):
    logger.info("前置步骤 ==>> 用户{}登录，返回信息为token：{}".format(username,token))
...
def login_fixture():
    ...
    api_root_url = base_data["init_admin_user"]["api_root_url"]
    payload = {
        "username": username,
        "password": password
    }
    loginInfo = requests.get(url=api_root_url,params=payload)
    token=loginInfo.json()['data']['token']
    step_login(username,token)
    yield token
